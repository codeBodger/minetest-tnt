short = 4 -- The amount of time between the ignition of a tnt block after none have been ignited in a while and when it explodes
long = 10 -- The minimum amount of time between the explosion of one tnt block and the explosion of another


-- some things for testing
-- core.register_chatcommand("tnt", {
-- 	params = "<size>",
-- 	description = "Create tnt cube of size",
...
-- 				local k = 0
-- 				while k < size do
-- 					core.set_node(vector.add(player:get_pos(),{x=i+3, y=j-size/2, z=k+3}),{name="tnt:tnt"})
-- 					k = k+3
-- 				end
-- 				j = j+3
-- 			end
...
-- 				local k = 0
-- 				while k < size do
-- 					core.set_node({x=i, y=j, z=k},{name="air"})
-- 					k = k+1
-- 				end
-- 				j = j+1
-- 			end
...
-- simplify getting the time in seconds
function now()
	return os.time()
end
-- time when the most recent tnt to be ignited will explode
furthestBoom = now()
		...
		for y = -radius, radius do
			...
			for x = -radius, radius do
				...
				if r <= radius then
					...
					if node == "tnt:tnt_burning" or node == "tnt:boom" or node == "air" then
					elseif node == "tnt:tnt" then
						core.set_node(p, {name="tnt:tnt_burning"})
					else
						local node_drops = minetest.get_node_drops(node, "")
						for _, item in pairs(node_drops) do
							add_drop(drops, item)
						end
...
function tnt.register_tnt(def)
	...
	minetest.register_node(":" .. name .. "_burning", {
		...
		on_construct = function(pos)
			minetest.sound_play("tnt_ignite", {pos = pos})
			if now() > furthestBoom then furthestBoom = now() + short -- if there are no future scheduled tnt explosions, do this one after 'short' seconds
			else furthestBoom = furthestBoom + long end               -- otherwise, do it 'long' seconds after the last scheduled explosion
			minetest.get_node_timer(pos):start(furthestBoom - now())
